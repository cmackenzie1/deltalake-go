...
func TestMetadata_MarshalUnmarshalJSON(t *testing.T) {
	tests := map[string]struct {
		...
		"empty": {
			...
			wantJSON: `{"metaData":{"id":"","format":{"provider":"parquet","options":{}},"schemaString":"","partitionColumns":[],"configuration":{}}}`,
		},
		"full": {
			...
			wantJSON: `{"metaData":{"id":"af23c9d7-fff1-4a5a-a2c8-55c59bd782aa","format":{"provider":"parquet","options":{}},"schemaString":"...","partitionColumns":[],"configuration":{"appendOnly":"true"}}}`,
		},
	}
	for name, test := range tests {
		t.Run(name, func(t *testing.T) {
			gotJSON, err := test.metadata.MarshalJSON()
			require.NoErrorf(t, err, "Metadata.MarshalJSON() failed with error: %v", err)
			...
			require.NoErrorf(t, err, "Metadata.UnmarshalJSON() failed with error: %v", err)
